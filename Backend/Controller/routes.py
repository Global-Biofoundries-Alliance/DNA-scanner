...
def filterResults():
    ...
    try:
        ...
        if not request.is_json:
            return {'error': 'Invalid filter request: Data must be in JSON '
                    'format'}
...
def getSearchResults():
    ...
    parameters per form data:
        size: The number of results to be shown (note that there might be less
            available
    ...
    try:
        ...
        return {"error":
                "Encountered error while fetching search results\n" + (
                    traceback.format_exc() if __debug__ else "")}
...
def getAvailableHosts():
    ...
    try:
        ...
        return {"error":
                "Encountered error while fetching list of available hosts\n" +
                (traceback.format_exc() if __debug__ else "")}


@app.route('/codon_optimization', methods=['POST'])
def setCodonOptimizationOptions():
    '''Set codon optimization options.'''
    try:
        request_json = request.get_json()
        ...
        return {"error": "Encountered error while setting codon optimization "
                "options\n" + (
                    traceback.format_exc() if __debug__ else "")}
