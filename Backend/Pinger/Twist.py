    ...
    def __init__(self, email, password, apitoken, eutoken, username, firstname, lastname, host='https://twist-api.twistbioscience-staging.com/', timeout=60):
        ...
        if(self.address == "No_Address"):
            raise AuthenticationError(
                "The given first name and last name do not have a shipping address for this account.")
...
class Twist(BasePinger):
    ...
    def __init__(self, email, password, apitoken, eutoken, username, firstname, lastname, host='https://twist-api.twistbioscience-staging.com/', timeout=60):
        ...
        try:
            ...
            raise UnavailableError("Request got an error: " + str(exc.message) +
    ...
    def searchOffers(self, seqInf):
        ...
        try:
            ...
            self.vendorMessage = [Message(MessageType.VENDOR_INFO, "Twist can only provide the total price and time of the synthesizable sequences. Price: " + str(
                amount) + " $ , Time: " + str(turnOverTime) + " BD")]
            self.running = False
    ...
    def order(self, offerIds):
        ...
        if payments:
            ...
            try:
                ...
                self.running = False
                raise UnavailableError("Request got an error: " + str(
                    exc.message) + "and status code = " + str(exc.status_code)) from exc
            except ValueError as error:
                raise UnavailableError("Quote couldn't be checked.") from error
            except InvalidInputError as err:
                self.running = False
