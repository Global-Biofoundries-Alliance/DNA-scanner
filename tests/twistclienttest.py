...
class TestTwistClient(unittest.TestCase):
    ...
    # API.
    twist = Twist.TwistClient(email, password, apitoken, eutoken,
    ...
    def test_static_results(self):
        ...
        if(len(accounts) >= 1):
            for acc in accounts:
                ...
                expectedAccountKeys = ['id', 'name', 'type', 'vat', 'tax_exempt_certificate',
                                       'tax_vat_document_url', 'account_manager', 'customer_duns']
        ...
        if(len(prices) >= 1):
            for element in prices:
                ...
                expectedPriceKeys = [
                    'product_code', 'unit_price', 'price_type', 'product_family', 'product']
                self.assertEqual(True, list(element.keys())
                                 == expectedPriceKeys)
        ...
        expectedUserDataKeys = ['id', 'name', 'account_id', 'account_name', 'allowed_features', 'first_name', 'last_name', 'ship_to_phone', 'email', 'invited_by', 'used_by_e_commerce', 'mobile_phone',
                                'home_phone', 'phone', 'verification_status', 'default_shipping_address', 'default_payment_method', 'inactive', 'password_expiration_days_ttl', 'order_via_distributor', 'distributor']
        ...
        if(len(addresses) >= 1):
            for add in addresses:
                ...
                expectedAddressKeys = ['id', 'first_name', 'last_name', 'address_type', 'state', 'state_name', 'street_1', 'street_2', 'zip_code', 'city', 'organization',
                                       'country', 'country_name', 'phone_number', 'verification_status', 'created_date', 'billing_email', 'google_place_id', 'jaggaer_id', 'is_default']
        ...
        if(len(payments) >= 1):
            for payment in payments:
                assert isinstance(add, dict)
                expectedPaymentKeys = ['id', 'name', 'type', 'verification_status', 'submitted_by', 'is_active', 'created_date', 'submitted_by_contact', 'billing_address', 'available_po_balance', 'open_balance', 'starting_po_balance',
                                       'invoiced_amount', 'open_orders_amount', 'purchase_order_reference', 'expiration_date', 'po_document_url', 'sent_via', 'total_past_utilized_balance', 'invoices', 'open_orders', 'po_date', 'is_default']
                self.assertEqual(True, list(payment.keys())
                                 == expectedPaymentKeys)
    ...
    def test_ClientFunctions(self):
        ...
        for cons in constructs:
            ...
            expectedConsKeys = ['sequences', 'name', 'type',
                                'insertion_point_mes_uid', 'vector_mes_uid', 'column', 'row', 'plate']
        ...
        for subCons in submittedConstructs:
            ...
            expectedSubConsKeys = ['id', 'type', 'name', 'vector_mes_uid', 'vector_type', 'insertion_point_mes_uid',
                                   'adapters_on', 'external_id', 'catalog_id', 'production_data', 'product_code', 'score']
        ...
        for score in scores:
            ...
            expectedScoreKeys = ['id', 'score_details', 'production_data', 'score_data', 'sequences', 'owner_contact', 'type', 'name', 'notes', 'score', 'scored', 'vector_mes_uid',
                                 'vector_type', 'insertion_point_mes_uid', 'is_insertion_site_id', 'container_id', 'created_at', 'updated_at', 'adapters_on', 'external_id', 'catalog_id', 'product_code']
        ...
        # This quote is NOT used in the final order. This quote gets deleted
        # later.
        ...
        with self.assertRaises(Twist.TwistError):
        ...
        self.assertEqual(
            True, {'quote_marked_for_deletion': example_id} == delete_quote)
        ...
        expecteFinalQuoteKeys = ['id', 'status_info', 'pdf_download_link',
                                 'quote', 'opportunity_id', 'q_number', 'tat', 'owner_contact']
        self.assertEqual(True, list(finalQuote.keys())
                         == expecteFinalQuoteKeys)
