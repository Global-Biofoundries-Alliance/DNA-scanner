...
class TestGeneArtClient(unittest.TestCase):
    ...
    # API.
    geneArt1 = GeneArt.GeneArtClient(server, validate, status, addToCart,
                                     upload, username_real, token_real, dnaStrings, hqDnaStrings, 60)
    ...
    def test_authenticate(self):
        ...
        with self.assertRaises(Exception):
            GeneArt.GeneArtClient(TestGeneArtClient.server, TestGeneArtClient.validate, TestGeneArtClient.status, TestGeneArtClient.addToCart,
                                  TestGeneArtClient.upload, username_dummy, token_dummy, TestGeneArtClient.dnaStrings, TestGeneArtClient.hqDnaStrings, TestGeneArtClient.timeout)

        self.assertEqual(True, self.geneArt1.authenticate())

    # Check the projectValidate method by checking the keys and their values
    # returned by the API.
    def test_projectValidate(self):
        ...
        for cons in response['constructs']:
            # Check the value of each key included in the
            # response['constructs']
    ...
    # 4. Review the status of the upload from step 1. (--> Test
    # 'statusReview'. "status" must now be "in the cart".)
    ...
    def test_other(self):
        ...
        self.assertEqual(True, toCartExpectedKeys ==
                         list(toCartResponse.keys()))
