...
class TestCompositePinger(unittest.TestCase):
    ...
    def test_is_running(self):
        print("--->>> Start test for: " +
              TestCompositePinger.name + " - isRunning()")
        ...
        p.registerVendor(Entities.VendorInformation(
            name="Dummy", shortName="Dummy", key=1), pingerDummy1)
        p.registerVendor(Entities.VendorInformation(
            name="Dummy", shortName="Dummy", key=2), pingerDummy2)
    ...
    def test_get_vendor(self):
        print("--->>> Start test for: " +
              TestCompositePinger.name + " - getVendor")
        # Create Dummy Pinger
        pingerDummy1 = DummyPinger()
        pingerDummy2 = DummyPinger()
        ...
        p.registerVendor(Entities.VendorInformation(
            name="DummyDuplicate", shortName="DummyDuplicate", key=2), pingerDummy2)
        self.assertEqual(2, len(p.getVendors()))
        vendor = p.getVendors()[1]
    ...
    def test_getorders(self):
        print("--->>> Start test for: " +
              TestCompositePinger.name + " - getOrders")
        ...
        p.searchOffers([Entities.SequenceInformation(
            "ACTG", "TestSequence", "ts1")])
        ...
        order = p.order(vendor=1, offerIds=[p.getOffers()[
                        0].vendorOffers[0].offers[0].key])
        ...
        # Filter Vendor 1
        p.searchOffers([Entities.SequenceInformation(
            "ACTG", "TestSequence", "ts1")], vendors=[1])
        self.assertEqual(1, len(p.getOffers()))
        self.assertEqual(1, len(p.getOffers()[0].vendorOffers))
        self.assertEqual(1, p.getOffers()[
                         0].vendorOffers[0].vendorInformation.key)
        self.assertEqual(1, len(p.getOffers()[0].vendorOffers[0].offers))

        # Filter Vendor 2
        p.searchOffers([Entities.SequenceInformation(
            "ACTG", "TestSequence", "ts1")], vendors=[2])
        self.assertEqual(1, len(p.getOffers()))
        self.assertEqual(1, len(p.getOffers()[0].vendorOffers))
        self.assertEqual(2, p.getOffers()[
                         0].vendorOffers[0].vendorInformation.key)
        self.assertEqual(1, len(p.getOffers()[0].vendorOffers[0].offers))

        # search with 2 sequences, 1 vendor with orders and 1 vendor without
        # orders
        p.searchOffers([Entities.SequenceInformation("ACTG", "TestSequence", "ts1"),
                        Entities.SequenceInformation("ACTG", "TestSequence", "ts2")])
        pingerDummy2.offers = []
        ...
        # Test that CompositePinger ignores output of a VendorPinger, if
        # invalid
    ...
    def testVendorOffers(self):
        ...
        p.registerVendor(Entities.VendorInformation(
            name="DummySuccess", shortName="DummySucc", key=1), successPinger)
        ...
        p.registerVendor(Entities.VendorInformation(
            name="DummyNotAvailable", shortName="DummyNA", key=2), unavailablePinger)
        ...
        p.registerVendor(Entities.VendorInformation(
            name="DummyRunning", shortName="DummyRunning", key=3), runningPinger)
